...
void setup() 
{
  ...
  pinMode(led_pin, OUTPUT);//13
  digitalWrite(led_pin, LOW);
  pinMode(fan_pin, OUTPUT);//13
...
unsigned char dp_process(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  switch(dpid) {
    ...
    break;
    case DPID_SWITCH:
      fan_state = my_device.mcu_get_dp_download_data(dpid, value, length); /* Get the value of the down DP command */
      if (fan_state) {
        //Turn on
...
{
  my_device.mcu_dp_update(DPID_LED, led_state, 1);
  my_device.mcu_dp_update(DPID_SWITCH, fan_state, 1);
}
